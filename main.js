...
const descriptions = [
    [
        "Bring a piece of history to the city's urban streets as you walk into Nike Jordan Max Aura 3 men's sneakers. Inspired by the rich Jordanian heirtage, this model has the energy of basketball shoes and a look that changes the preception of the classic style.",
    ],
    [
        "Nike Air Max 95 men's sneakers move you with the strength and fluidity inspired by the anatomy of the human body. The central sole forms the basis of these sneakers, while the structured side panels give a solid and stable sonctruction. Flexible incisions in the sole allow you feet to move naturally.",
    ],
    [
        "Jordan Delta 2 SP men's basketball shoes offer a fresh and fearless approach to the characteristics you want: durablitly, comfort and the attitude of hte Jordan brand. The first model of Delta 2 sneakers, with the smae idea, received redesigned lines and modified components.",
    ],
];
...
// Retriving image form folder path
...
for (let i=0; i<thumb.length; i++){
    thumb[i].addEventListener("click", (e)=>{
        // Js arrays start with 0, so we set the id to i+1
        id=i+1;

        // Setting the main image to clicked thumbanail image
...
function slider(shoe){
    ...
    // Adding all of the animations
    animate(img, 600, "replace 600ms ease-in");
    animate(shadow, 600, "shadow2 600ms ease-in");
    animate(titleOverlay, 850, "title 800ms ease");
}
...
next.addEventListener("click", ()=>{
    // Increment img id
    shoe++;
    // Check if slider goes above the sliders length, and reset it to the first
    if(shoe >pag.length){
        shoe=1;
    }
    //Run the slider function
